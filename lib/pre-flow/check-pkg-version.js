...
function isUpToDate(callback) {
  ...
  request.get('https://registry.npmjs.org/kcv-cli', function(err, res) {
    if (err) {
      return callback(err);
    }
    ...
    if (localVersion === latestnpmVersion) {
      console.log('Your kcv-cli software is up-to-date.');
    } else {
      console.log(chalk.yellow('Notice: You are currently using an out-of-date version of kcv-cli.'));
      console.log(chalk.cyan('Type'), '`sudo npm update -g kcv-cli`', chalk.cyan('to upgrade to version'), latestnpmVersion);
    }
