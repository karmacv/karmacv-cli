...
function isUpToDate(callback) {
  ...
  request.get('httpS://registry.npmjs.org/kcv-cli', function(res) {
    if (res.error) {
    ...
    if (localVersion === latestnpmVersion) {
      ...
      console.log('Notice: You are currently using an out-of-date version of kcv-cli.'.yellow);
      console.log('Type'.cyan, '`sudo npm update -g kcv-cli`', 'to upgrade to version'.cyan, latestnpmVersion);
    }
