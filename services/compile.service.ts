...
export class CompileService {
  ...
  compileDocx(themePath: string, json: string) {
    return this.compileHTML(themePath, json, DocType.PDF).then((html) => {
      return htmlToDocxBuffer(html, null, {
  ...
  embedRenderedHTML(
    ...
    var mainLayout = Deno.readTextFileSync(
      `/${dirname}/../server/templates/layout.eta`,
    );
