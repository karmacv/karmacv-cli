...
export class ProgramService {
    ...
    init() {
        ...
        this.program
            .command({name: 'serve', description: `Serve resume in live browser`, fn: (arg) => {
                ConfigService.modulePath = arg._[0] as string;
                console.log(arg._[0] as string);
                this.utilsService.startApp();
            }})
            .argument({ name: "themePath", multiple: false, optional: false, description: 'URL or local path to theme path.' });
